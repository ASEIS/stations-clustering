...
\response{%
We are a little surpirsed by this comment. The paper does have an explanation about confusion matrices. In the original manuscript, this explanation is the section titled \textbf{Decision Trees}, page 13, between lines 280 and 282. The explanation includes an illustrative example for the simplest case of two categories and it should not be difficult for any reader of the BSSA to understand this concept is extendable to larger matrics such as those shown in Tables 2 and 3. We added a sentence in the explanation to make it explicit that they can be extended to deal with more attributes.
}
...
\response{%
We disagree. In our view, these sentences are written as indirect questions and do not require the use of a question mark. To make this clearer, we modified the beginning of the sentences slightly by removing the colon and adding the word ``regarding''.
}
