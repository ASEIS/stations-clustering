...
\response{%
That is right. It should be the number of decision attributes. We modified the figure accordingly. The reason why there is no data for 1 is because there are no trees that would classify the data using a single metric into the four classes because these are binary trees, i.e., each node represents a binary decision, so one needs at least 2 metrics with 3 or more nodes with a minimum depth of 2. On the other end of things, the reason why there is no data for 11 metrics is because the analysis of the data resulted in the metric C11 not having any participation in the decision process, as shown in the original Table 6. In new version of Table 6 we include cases in which we forcefully removed metric C8, first, and then C5 and C8. Once one does that, the participations of the metrics changes and so would the topology of the trees.
}
...
\response{%
We also mention in the conclusion that the next metrics in line are the energy (C4), the peak acceleration (C5) and peak velocity (C6). In order to emphasize this point, as described in the response to comment \#8, we also included a new table (Table 7) showing the participation of the different metrics in the absence of the response spectra (C8) and the peak acceleration (C5). The results confirm that in such cases the scond most relevant metrics are still the energy (C4) and the peak ground velocity (C6) as mentioned before. This additional analysis also confirms the very low or null participation of the duration (C11) in other trees.
}
...
\response{%
We disagree about the need to make detailed distinctions about these different algorithms. That is why we provided references. If the reader is interested in learning the differences, the most we can do without expanding the scope of our study is to point him/her in the right direction, which we already do. Nonetheless, in attention to the Reviewer, we modified slightly the descritpion to give some additional context in what regards to the C5.0 algorithm and added a sentence explaining that detailing the alternative algorithms is out of the scope of this study.
}
...
\response{%
Following Equation (3) we define $A[a]$ as all the possible values of attribute $A$. The use of $[a]$ implies that $A$ is considered a numerically discrete attribute. We therefore meant to say that if $A$ continues attribute. Thanks for the comment. Fixed.
}
