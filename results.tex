...
% As we mentioned in reference to the results shown in Fig.~\ref{nodes-depth}, all trees led to relatively high values of the effectiveness score (i.e., $F_1 > 0.9$). As an example, Table \ref{tab:t1:confusion} shows the count of samples in the confusion matrix for tree T1 and Table \ref{tab:t1:f1} show the results of the precision and recall factors, along with the individual-leaves and leaves average $F_1$ scores.

% All trees having high $F_1$ factors, we highlight (with darker color) the bars in those histogram where a dominance of a class is strong to help us understand their differences. 
