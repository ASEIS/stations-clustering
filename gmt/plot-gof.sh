...
THESCORES=${DATADIR}/data4shell_NS_CVMS.txt
...
awk -F' ' '{print $3, $3, $'$COLUMN'}' $THESCORES > $SCORES
...
gmt psbasemap -J -Ba20f10:"Count":W -R0/10/0/$MAXCOUNT -K -O >> $OUTPUT
...
#grdimage gof.grd -J -B -C$CPTFILE -V -K -O >> $OUTPUT
...
gmt psxy $EPICENTER -R -J -Sa10p -Gblack -Wthin -V -K -O >> $OUTPUT

gmt gmtset MAP_TICK_LENGTH_PRIMARY = 0p
gmt psbasemap -J -Bf180000wesn -R -K -O >> $OUTPUT

# Socres vs epicentral distance
# ------------------------------------------------------------------------------
